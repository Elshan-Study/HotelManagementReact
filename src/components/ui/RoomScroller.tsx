            ...
            <div ref={scrollRef} className="flex gap-5 py-2 overflow-x-auto snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
                {items.map((item) => (
                    <Link to="/rooms" key={item.id}
                          ...
                          style={{ scrollbarWidth: "none", "--cols": "5", "--gap": "20px" } as React.CSSProperties}
                    >
                        <span className="text-6xl">{item.emoji}</span>
                        <span className="font-georgia font-bold text-stone-700 text-xl text-center">{item.label}</span>
                        <span className="text-sm text-stone-500 text-center px-2">{item.desc}</span>
                        <span className="text-sm text-amber-700 font-semibold bg-amber-100 px-4 py-1.5 rounded-full mt-1">{item.price}</span>
                    </Link>
                ))}
            </div>
        </div>
